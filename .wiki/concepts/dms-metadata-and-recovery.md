...
v0.10.0은 `PublicDocumentMetadata`와 `DocumentMetadata`를 구조적으로 분리한다. 일반 upload/get/list/content 결과에는 `storage_key`가 없고, `public_metadata()`·`to_public_dict()`·JSON schema도 public-safe projection을 유지한다. `DocumentMetadata.storage_key`는 `get_internal_document_metadata()`, `inspect_document()`, recovery candidate와 reconciliation 같은 명시적 관리 경계에서만 사용한다.^[raw/articles/dms-core-api-reference-v0.10.0-2026-08-25.md]^[raw/articles/dms-core-examples-v0.10.0-2026-08-25.md]

`DocumentMetadata`를 일반 HTTP 응답, operation event, tenant callback에 그대로 전달하지 않는다. `to_dict()`는 기존 `extra_metadata` field명을 유지하고 `to_public_dict()`는 외부 canonical `metadata` field를 사용한다. public schema와 canonical dump에는 `storage_key`가 없어야 한다.^[raw/articles/dms-core-api-reference-v0.10.0-2026-08-25.md]

## Application-owned metadata

v0.10.0의 upload metadata와 scoped default metadata는 `object` 타입의 application-owned 값이다. DMS는 metadata 업무 schema, 민감 key 차단, JSON serialization, normalization을 정의하거나 검증하지 않으며 입력 값을 idempotency fingerprint에도 포함하지 않는다. 따라서 기존 v0.7.0 문서에 있던 `DefaultMetadataPolicy`·`StructuredMetadataValidator` 전제는 v0.10.0 current API에 자동 적용되는 계약으로 간주하지 않는다.^[raw/articles/dms-core-api-reference-v0.10.0-2026-08-25.md]
...
## v0.10 user-scoped management boundary

`PublicDocumentMetadata`와 upload result에는 `user_id`가 포함될 수 있지만 `storage_key`는 계속 제외된다. `AccessContext.user_id`와 `DmsOperationContext.user_id`는 public metadata, internal metadata, list cursor, recovery, reset에 같은 user namespace를 적용한다. 다른 user의 document ID를 privileged read/delete/recovery에 사용하면 `AccessDeniedError`다.^[raw/articles/dms-core-api-reference-v0.10.0-2026-08-25.md]^[raw/articles/dms-core-examples-v0.10.0-2026-08-25.md]
