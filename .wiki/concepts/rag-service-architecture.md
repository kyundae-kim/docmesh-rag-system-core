...
DocMesh RAG Core의 현재 구조는 외부에 `RAGCore` 단일 진입점을 유지하면서 내부를 IngestionService, RetrievalService, GenerationService, MetadataStore, DocumentStorage, Milvus Lite vector store로 분리하는 형태다. 현재 PRD는 이 구성을 **조립 가능한 Python RAG 라이브러리**의 기준선으로 정의하고, SRS는 이를 구현·검증 가능한 소프트웨어 경계로 정리한다.^[raw/articles/docmesh-rag-core-prd-2026-06-23.md]^[raw/articles/docmesh-rag-core-srs-2026-06-23.md]
...
현재 아키텍처 설명에는 `bootstrap_rag_core`, `DocmeshRAGServiceFactory`, `create_rag_embedding_client`, `create_rag_generation_client`, `create_rag_vector_store`, `create_rag_metadata_store`, `create_rag_document_storage`, `create_rag_chunker`, `load_docmesh_settings`, `resolve_user_id`가 함께 포함된다. SRS는 여기에 `create_service_registry(...)`까지 외부 composition interface로 명시하므로, 이 프로젝트에서 composition은 단순 편의 함수 모음이 아니라 [[public-api-surface]]와 [[service-factory-registry]]를 잇는 조립 경계다.^[raw/articles/docmesh-rag-core-prd-2026-06-23.md]^[raw/articles/docmesh-rag-core-srs-2026-06-23.md]

## Evolution path

이 아키텍처는 현재 SDK 우선 설계를 취하지만, PRD는 이후 FastAPI 기반 서비스 래핑, async ingestion, 외부 vector DB adapter, richer document parsing 같은 확장 가능성을 별도 후속 범위로 둔다. 따라서 현재 제품 계약과 미래 확장을 구분하려면 [[product-scope-and-requirements]]와 [[interface-roadmap]]를 함께 보는 편이 정확하고, 요구사항 추적 관점에서는 [[software-requirements-and-traceability]]를 추가로 봐야 한다.^[raw/articles/docmesh-rag-core-prd-2026-06-23.md]^[raw/articles/docmesh-rag-core-srs-2026-06-23.md]
