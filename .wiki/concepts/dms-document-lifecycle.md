...
metadata의 타입은 `object`이며 DMS가 업무 schema·보안 규칙·정규화·JSON 직렬화를 정의하거나 검증하지 않는다. metadata는 upload idempotency fingerprint에서 제외되므로 외부 응답이나 메시지로 내보낼 때의 serialization과 secret 검사는 host 책임이다.^[raw/articles/dms-core-api-reference-v0.10.0-2026-08-25.md]^[raw/articles/dms-core-examples-v0.10.0-2026-08-25.md]
...
`DmsOperationContext`는 access context, created_by, idempotency scope, audit actor, default metadata를 scoped facade의 기본값으로 제공한다. 작업에 명시한 값이 context 기본값보다 우선하고, scoped facade는 shared SDK lifecycle을 소유하지 않는다.^[raw/articles/dms-core-api-reference-v0.10.0-2026-08-25.md]
...
## v0.10 native async and user scope

native async factory는 async storage component를 사용하고 `create()`는 lazy SDK, `create_async()`는 ready SDK를 반환한다. 기존 sync SDK를 `AsyncDocumentManagementSDK(sync_sdk)`로 감싸는 compatibility 경계와 native async assembly는 서로 다른 construction path다.^[raw/articles/dms-core-api-reference-v0.10.0-2026-08-25.md]^[raw/articles/dms-core-examples-v0.10.0-2026-08-25.md]

`AccessContext.user_id`와 `DmsOperationContext.user_id`는 문서 metadata, object namespace, idempotency operation, cursor에 동일 scope를 적용한다. scoped facade에서 context와 다른 user ID를 명시하거나 다른 user document에 접근하면 validation/access error가 발생하며, user-scoped reset은 해당 user의 DMS 관리 데이터만 대상으로 한다.^[raw/articles/dms-core-api-reference-v0.10.0-2026-08-25.md]
