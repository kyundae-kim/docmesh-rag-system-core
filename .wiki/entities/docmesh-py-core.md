...
`docmesh-py-core`는 DocMesh 계열 Python 서비스가 반복적으로 구현하던 설정 로드, 외부 서비스 client 생성, health check, Keycloak 인증, 민감정보 마스킹을 공통 SDK로 묶어 소비 프로젝트에서 재사용하게 하는 기반 패키지다. v0.5.0의 일반 애플리케이션 경로는 **RuntimePlan 기반 assembly-first**다. 동기 서비스는 `assemble_services()`, NATS를 포함한 비동기 lifecycle은 `await assemble_service_runtime(plan=...)`으로 선택 설정 로드·필수/대안 서비스 검증·client 생성·startup healthcheck를 조립하고, 반환된 `ServiceBundle`/`ServiceRuntime` context manager가 lifecycle을 관리한다.^[raw/articles/docmesh-py-core-api-reference-v0.5.0-2026-07-27.md]
...
v0.5.0 API reference는 패키지 루트 import 경계를 `__all__` 기준으로 명시하며 86개 공개 이름을 계약 테스트로 검증한다. `RuntimePlan`, `Service`, `ServiceConfigs`, `ServiceBundle`, `ServiceRuntime`, `ServiceClientWrapper`, 각 `*Config`, `assemble_services`, `assemble_service_runtime`, `load_service_configs`, `load_available_service_configs`, `diagnose_services`, health/cleanup helper 및 Keycloak API가 안정적인 공개 소비면이다. 하위 모듈 직접 import보다 이 루트 경계를 사용해야 한다.^[raw/articles/docmesh-py-core-api-reference-v0.5.0-2026-07-27.md]

## Configuration policy

v0.5.0 설정 계약은 환경변수에서 공백·boolean·숫자형을 엄격히 검증하고, `load_service_configs(services={...})`/`load_available_service_configs(services={...})`로 필요한 서비스만 검증하게 한다. production 판정은 `DOCMESH_SECURITY_MODE` 또는 `DOCMESH_ENV`와 alias 목록으로 결정하며 Keycloak·MinIO·Milvus·Ollama의 전송 보안 제약을 강제한다. 이 운영 규칙과 `RuntimeDefaults`를 통한 설정 보존은 [[settings-loading-and-validation]] 및 [[service-configuration-topology]] 수준의 설계 제약이다.^[raw/articles/docmesh-py-core-configuration-v0.5.0-2026-07-27.md]
