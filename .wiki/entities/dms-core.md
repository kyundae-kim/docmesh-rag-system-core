...
`dms-core`는 문서 metadata와 object storage를 하나의 document-management facade로 묶어 업로드·조회·삭제·reset·정합성 복구를 제공하는 Python SDK다. 최신 ingest 기준은 v0.10.0이며, 권장 소비 경계는 `from dms import ...`다. SDK는 host가 만든 SQLAlchemy `Engine`, MinIO client, 또는 storage component를 주입받고, DMS는 독립 실행형 API 서버가 아니며 주입 자원의 생성·readiness·종료도 소유하지 않는다.^[raw/articles/dms-core-api-reference-v0.10.0-2026-08-25.md]

## v0.10.0 public surface

기본 조립은 `DocumentManagementSDKFactory(engine=..., minio_client=..., bucket_name=...)`이고, 이미 준비된 adapter를 사용할 때는 `DefaultDocumentManagementSDK(metadata_store=..., object_store=..., operation_store=...)`를 직접 만든다. v0.10.0 package root에는 `dms.__all__` 기준 55개 공개 이름이 있고 `dms.sdk.__all__`은 54개다. sync facade는 26개, native async facade는 28개 public member를 가지며, sync/async scoped facade는 각각 25개 작업 member를 제공한다.^[raw/articles/dms-core-api-reference-v0.10.0-2026-08-25.md]^[raw/articles/dms-core-examples-v0.10.0-2026-08-25.md]

현재 공개 계약에는 환경변수에서 client를 만드는 factory, `check_health()`, facade 전역 `close()`/`aclose()`, HTTP response 모델, 내부 store 구현이 포함되지 않는다. native async 조립은 별도 `AsyncDocumentManagementSDKFactory`의 `create()`/`create_async()` 경계를 사용하고, sync factory에 `create_async()`가 있다고 가정하지 않는다. 따라서 v0.7.0에 기록된 `DmsAssemblyPlan`·resource ownership·health facade를 v0.10.0 runtime API로 재사용하지 말고, host composition과 transport adapter에서 별도로 책임져야 한다.^[raw/articles/dms-core-api-reference-v0.10.0-2026-08-25.md]
...
v0.10.0에서는 `AccessContext.user_id`와 `DmsOperationContext.user_id`가 문서 정보, object namespace, idempotency operation, cursor 조건에 같은 사용자 범위를 적용한다. scoped facade는 context의 access와 기본값을 자동 전달하고, 다른 user scope에 대한 조회·본문·삭제·복구는 `AccessDeniedError`로 거부한다.^[raw/articles/dms-core-api-reference-v0.10.0-2026-08-25.md]^[raw/articles/dms-core-examples-v0.10.0-2026-08-25.md]

## Version transition and evidence boundary

v0.6.0과 v0.7.0 raw 문서는 과거 환경/service-config factory 및 assembly-plan 중심 설계를 기록한다. v0.9.0 API/Examples 문서는 과거 release 기준으로 보존하고, 최신 v0.10.0 API/Examples 문서는 `dms-core` commit `d508b7c2ea82fb79bfcf008c948a364fcaa962d9`, package version `0.10.0`을 기준으로 한다. persistent idempotency replay/conflict/lookup 일부는 v0.10.0 추적성 matrix에서도 `source-only` gap임을 명시한다. 이 페이지는 GitHub Wiki 문서 기반이며 설치 package나 소비 repository를 live 실행해 검증한 결과는 아니다.^[raw/articles/dms-core-api-reference-v0.10.0-2026-08-25.md]
